...
class PPORLHFTrainer(BaseTrainer):
    ...
    def __init__(self, config: PPOConfigBase):
        ...
        self.reward_model = (
            HFModel_Reward.init_from_hf_pretrained(self.config)
            .to(self.device)
            .requires_grad_(False)
        )
        ...
        self.data = TLDRFilteredDataPPO(
            tokenizer=self.policy_model.tokenizer, batch_size=self.config.batch_size
        )
        ...
        if self.config.resume_from_checkpoint:
            ...
            self._load_from_checkpoint(
                ...
                value_checkpoint_path=self.config.value_checkpoint_path,
            )
    ...
    def _log(self, loss_ppo, loss_value, k, old_data, ppo_log_data):
        ...
        self.logger.log(
            scalars={
                ...
                / masked_var(
                    old_data["A"] + old_data["values"][:, :-1],
                    old_data["action_pad_mask"],
                ).item(),  # 1 - var(A) / var(A + V)
                # State stats
                ...
                "mean_raw_reward": masked_mean(
                    old_data["raw_rewards"], old_data["reward_mask"]
                ).item(),
                ...
                "kl_mean": old_data["kl"]
                .masked_fill(~old_data["action_pad_mask"], 0)
                .sum(1)
                .mean()
                .item(),
                ...
                "unique_tokens_per_response": torch.tensor(
                    [len(torch.unique(resp)) for resp in old_data["states"]]
                )
                .float()
                .mean()
                .item(),
    ...
    def _forward(self, states):
        new_values = self.value_model.forward(
            states, max_query_length_truncate=self.data.SFT_MAX_QUERY_LENGTH - 1
        ).squeeze(1)
        new_policy_logits, _ = self.policy_model.forward(
            states, max_query_length_truncate=self.data.SFT_MAX_QUERY_LENGTH - 1
        )
    ...
    @detect_nans
    def _compute_policy_loss_ppo(
        self, old_actions, old_log_probs, A, new_policy_logits, action_pad_mask
    ):

        old_log_probs = old_log_probs.detach()
        A = A.detach()

        new_log_policies = masked_log_softmax(
            new_policy_logits, action_pad_mask.unsqueeze(2), mask_value=0, dim=-1
        )
        ...
        loss_ppo = torch.min(
            ...
            torch.clamp(
                ratios,
                1 - self.config.eps_policy_clipping,
                1 + self.config.eps_policy_clipping,
            )
            * A,
        )
        ...
        with torch.no_grad():
            ...
            entropy_per_token = -torch.sum(
                new_log_policies_unmasked * torch.exp(new_log_policies_unmasked), dim=-1
            )
        ...
        for epoch in range(self.config.num_epochs):
            for batch_idx, batch in enumerate(self.data.train_loader):
                ...
                tj_loader = DataLoader(
                    TrajectorySet(tjs),
                    batch_size=self.config._mini_batch_size,
                    shuffle=False,
                    num_workers=0,
                )
                    ...
                    for _, old_data in enumerate(tj_loader):
                        ...
                        with self.mixed_precision_context:
                            ...
                            self.loss_value = self._compute_value_loss_mse(
                                ...
                                new_values,
                                old_data["values"],
                                old_data["value_pad_mask"],
                            )

                            # 2.2 Compute ppo loss for policy model
                            self.loss_ppo, ppo_log_data = self._compute_policy_loss_ppo(
                                old_data["actions"],
                                old_data["log_probs"],
                                old_data["A"],
                                new_policy_logits,
