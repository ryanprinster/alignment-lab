                    ...
                    for _, old_data in enumerate(tj_loader):
                        ...
                        self.logger.log(
                            scalars={
                                ...
                                "token_diversity_ratio": (torch.tensor([len(torch.unique(resp[old_data['action_pad_mask'][i]])) for i, resp in enumerate(old_data['states'])]).float() / old_data['action_pad_mask'].float().sum(1)).mean().item(),
