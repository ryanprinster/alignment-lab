...
class SFTEval(BaseTrainer):
    def __init__(self, config: SFTConfigBase):
        ...
        self.sft = HFModel_SFT.init_from_hf_pretrained(self.config).to(self.device).requires_grad_(False)
        self.sft.set_from_local_state_dict(self.config.sft_model_path)
        self.gpt = HFModel_SFT.init_from_hf_pretrained(self.config).to(self.device).requires_grad_(False)
    ...
    def evaluate(self):
        for _batch_idx, batch in enumerate(self.data.test_loader):
            for subreddit, title, post, summary in zip(batch["subreddit"], batch["title"], batch["post"], batch["summary"]):
                ...
                with torch.no_grad():
                    sft_gen_ids, _ = self.sft.generate(inputs, TLDRFilteredDataSFT.SFT_MAX_INPUT_LENGTH, self.config.generation_temperature, do_sample=False)
                    gpt_gen_ids, _ = self.gpt.generate(inputs, TLDRFilteredDataSFT.SFT_MAX_INPUT_LENGTH, self.config.generation_temperature, do_sample=False)
