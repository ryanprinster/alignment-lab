...
class Checkpointer:
    ...
    def save_checkpoint(
        ...
        should_save_checkpoint, path = self._should_save_checkpoint(
            global_step, loss, checkpoint_prefix, final_checkpoint
        )
    ...
    def _should_save_checkpoint(
        ...
        if global_step % self.save_freq_steps == 0 and global_step != 0:
            path = os.path.join(
                self.checkpoint_dir,
                f"{checkpoint_prefix}_checkpoint_step_{global_step}.pt",
            )
            should_save_checkpoint = True
    ...
    def load_checkpoint(self, checkpoint_path, model, device, optimizer=None):
        ...
        print(
            f"Checkpoint loaded - Step: {training_state['global_step']}, "
